...
def _get_chia_flags(size, memory_buffer, temporary_directory, destination_directory, threads, buckets, bitfield,
    ...
    if pool_contract_key is not None:
        flags['c'] = pool_contract_key
...
def _get_madmax_flags(temporary_directory, destination_directory, threads, buckets, buckets3,
                      temporary2_directory=None, farmer_public_key=None, pool_public_key=None,
                      pool_contract_key=None, **kwargs):
    flags = dict(
        r=threads,
        t=temporary_directory,
        d=destination_directory,
